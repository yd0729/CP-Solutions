...
## 467. 环绕字符串中唯一的子字符串（DP） ⭐️

由于 s 是周期字符串，对于在 s 中的子串，只要知道子串的第一个字符（或最后一个字符）和子串长度，就能确定这个子串。例如子串以 
‘d’
‘d’ 结尾，长度为 
3
3，那么该子串为 
...
题目要求不同的子串个数，那么对于两个以同一个字符结尾的子串，长的那个子串必然包含短的那个。例如 
“abcd”
“abcd” 和 
“bcd”
“bcd” 均以 
...
α 结尾且在 
s
s 中的子串的最长长度，知道了最长长度，也就知道了不同的子串的个数。
...
p[i−1] 在字母表中的下一个字母，则将 
k
k 加一，否则将 
k
k 置为 
1
1，表示重新开始计算连续递增的子串长度。然后，用 
k
k 更新 
dp
[
