...
## 68 - II. 二叉树的最近公共祖先（二叉树） ⭐️

## 68 - I. 二叉搜索树的最近公共祖先（二叉树） ⭐️
...
## 56 - II. 数组中数字出现的次数 II（位运算） ⭐️

题解来自大佬「[Krahets](https://leetcode.cn/problems/shu-zu-zhong-shu-zi-chu-xian-de-ci-shu-ii-lcof/solution/mian-shi-ti-56-ii-shu-zu-zhong-shu-zi-chu-xian-d-4/)」．

基本思路就是统计每 1 位上 1 出现的次数（通法）．
...
## 56 - I. 数组中数字出现的次数（位运算） ⭐️

1. 先对所有数字进行一次异或，得到两个出现一次的数字的异或值。
2. 在异或结果中找到任意为 1 的位。
3. 根据这一位对所有的数字进行分组（可以保证出现 1 次的数备份在不同的组中，问题转化为有 1 个数出现 1 次，其他数出现 2 次）．
...
## 34. 二叉树中和为某一值的路径（二叉树、DFS） ⭐️

## 33. 二叉搜索树的后序遍历序列（BST） ⭐️
