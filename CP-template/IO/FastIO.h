...
class inputHelper {
  ...
  public:
    ...
    template <typename _Tp,
              std::enable_if_t<std::is_signed_v<_Tp> & std::is_integral_v<_Tp>>
                  * = nullptr>
    ...
    template <typename _Tp,
              std::enable_if_t<std::is_unsigned_v<_Tp> &
                               std::is_integral_v<_Tp>> * = nullptr>
    inputHelper<_BufferSize, _BlockSize> &operator>>(_Tp &ret) {
        while (isBlank(getChar_Checked()))
            next();
        flush();
    ...
    template <typename _Tp,
              std::enable_if_t<std::is_floating_point_v<_Tp>> * = nullptr>
    inputHelper<_BufferSize, _BlockSize> &operator>>(_Tp &ret) {
...
inputHelper<_BufferSize, _BlockSize> &
getline(inputHelper<_BufferSize, _BlockSize> &ih, std::string &ret) {
    ret.clear();
    if (ih.getChar_Checked() == EOF)
        ih.setState(false);
    else {
        while (!inputHelper<_BufferSize, _BlockSize>::isEndline(
            ih.getChar_Checked())) {
